...
const uint8_t* wifista_ssid="ALIENTEK";			//路由器SSID号
const uint8_t* wifista_encryption="wpawpa2_aes";	//wpa/wpa2 aes加密方式
const uint8_t* wifista_password="15902020353"; 	//连接密码
...
//键盘码表
const uint8_t* kbd_tbl[13]={"1","2","3","4","5","6","7","8","9",".","0","#","DEL"}; 
uint8_t* kbd_fn_tbl[2];
//加载键盘界面（尺寸为240*140）
...
void atk_8266_load_keyboard(uint16_t x,uint16_t y)
{
	...
	for(i=0;i<15;i++)
	{
		if(i<13)Show_Str_Mid(x+(i%3)*80,y+6+28*(i/3),(uint8_t*)kbd_tbl[i],16,80);
...
{		  
	...
	if(j&&(i>3))Show_Str_Mid(x+j*80,y+6+28*i,(uint8_t*)kbd_fn_tbl[keyx-13],16,80);
	else Show_Str_Mid(x+j*80,y+6+28*i,(uint8_t*)kbd_tbl[keyx],16,80);		 		 
}
//得到触摸屏的输入
//x,y:键盘坐标
...
{
	uint16_t i,j;
	static uint8_t key_x=0;//0,没有任何按键按下；1~15，1~15号按键按下
	uint8_t key=0;
	tp_dev.scan(0); 		 
...
void atk_8266_msg_show(uint16_t x,uint16_t y,uint8_t wanip)
{
	u8 *p,*p1,*p2;
	p=mymalloc(SRAMIN,32);							//申请32字节内存
	p1=mymalloc(SRAMIN,32);							//申请32字节内存
	...
	if(wanip==0)//全更新
	{
		...
		*p1=0;
		...
		Show_Str(x+72,y+16,240,16,(uint8_t*)ATK_ESP8266_CWMODE_TBL[*(p+1)-'1'],16,0);
  	atk_8266_send_cmd("AT+CWSAP?","+CWSAP:",20);	//获取wifi配置
		...
		*p1=0;
		...
		p=(uint8_t*)strstr((const char*)(p2+1),"\"");
		p1=(uint8_t*)strstr((const char*)(p+1),"\"");
		p2=p1;
		*p1=0;		
		...
		*p1=0;
		...
		Show_Str(x,y+80,240,16,"加密方式:",16,0);Show_Str(x+72,y+80,240,16,(uint8_t*)ATK_ESP8266_ECN_TBL[*(p1+1)-'0'],16,0);
	}
	myfree(SRAMIN,p);		//释放内存 
	myfree(SRAMIN,p1);		//释放内存 
	myfree(SRAMIN,p2);		//释放内存 
...
//1,TCP客户端
//2,UDP模式
uint8_t atk_8266_netpro_sel(uint16_t x,uint16_t y,uint16_t* name) 
{
	uint8_t key=0,t=0,*p;
	uint8_t netpro=0;
	LCD_Clear(WHITE);
